/*
 * minor modifications made to concat original source files into a single global object:
 *   https://github.com/MikeMcl/bignumber.js/blob/v1.4.1/bignumber.js
 *   https://github.com/sidorares/json-bigint/blob/v0.1.4/lib/parse.js
 *   https://github.com/sidorares/json-bigint/blob/v0.1.4/index.js
 */
...
var BigNumber, json_parse, json_bigint;
	...
	*/
	...
	function ifExceptionsThrow(arg, i, j, isArray, isRange, isErrors) {
		...
		if (ERRORS) {
			...
			message = ([
			method + ' number type has more than 15 significant digits', method + ' not a base ' + j + ' number', method + ' base' + message, method + ' not a number'][i] || j + '() ' + i + (isErrors ? ' not a boolean or binary digit' : message + (isArray ? ' or not [' + (outOfRange ? ' negative, positive' : ' integer, integer') + ' ]' : ''))) + ': ' + arg;
	...
	escapee = {
		...
		number = function () {
			...
			if (!isFinite(number)) {
				...
				if (string.length > 15) return new BigNumber(string);
				return number;
			}
		},
...
// ************************************************************************************************
json_bigint = function(options){
	this.parse = json_parse(options);
};

//create the default method members with no options applied (for backwards compatibility)
json_bigint.parse = json_parse();
// ************************************************************************************************
}
catch(e){
	BigNumber	= undefined;
